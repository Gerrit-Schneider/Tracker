...
class TrainingDetailsControllerIntegrationTest {
    ...
    void createsRunningDetailsAndCalculatesPace() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.runningDetails.runType")
                                .value("EASY")
                )
                .andExpect(
                        jsonPath("$.runningDetails.distanceMeters")
                                .value(5000)
                )
                .andExpect(
                        jsonPath(
                ...
                .andExpect(
                        jsonPath("$.runningDetails.averageHeartRate")
                                .value(148)
                )
                .andExpect(
                        jsonPath("$.runningDetails.maxHeartRate")
                                .value(169)
                );
    ...
    void createsBoulderingGradeResults() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isCreated())
                .andExpect(
                        jsonPath("$.boulderingResults.length()")
                                .value(2)
                )
                .andExpect(
                        jsonPath("$.boulderingResults[0].grade")
                                .value("V4")
                )
                .andExpect(
                        jsonPath(
                ...
                .andExpect(
                        jsonPath("$.boulderingResults[1].grade")
                                .value("V5")
                );
    ...
    void createsStrengthExercisesAndSets() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(status().isCreated())
                .andExpect(
                        jsonPath("$.strengthExercises.length()")
                                .value(1)
                )
                .andExpect(
                        jsonPath(
